...
All four are live-reproduced this engagement, not inferred.
...
Component: `schema_types.py` (validation), `schema_manager.py` (`schema sync`), `generator.py` (Python/TypeScript codegen)

- **Problem:** A schema field with `secret = true` and a real `defaultValue` is accepted with no rejection anywhere.
- **Current behavior:** The value is written verbatim into `.env.example` (`schema sync`), into generated Python (`pydantic-settings` field default), and into generated TypeScript (`z.string().default(...)`) .
- **Expected behavior:** Rejected at schema-load/validation time, or at minimum defensively omitted from every sync/generation path.
- **Evidence/reproduction:** Live-reproduced with a synthetic marker (`SYNTHETIC_NOT_A_SECRET_123` / a fake value) across all three outputs in an isolated `/tmp` project — confirmed present in `.env.example`, `config_generated.py`, and `config_generated.ts` verbatim.
- **Impact:** `.env.example` and generated config modules are exactly the files EnvShield's own workflow instructs a user to commit. Directly violates the charter's own §4 invariant ("never store secret values in the configuration contract").
- **Dependencies:** None.
- **Related:** BL-070 (the DEV.to comment that first raised the conceptual question this bug concretely answers).
- **Decision:** Open. Immediate fix candidate, ahead of general triage.
- **Target:** Before any release announcement.
- **Notes:** This directly contradicted CLAUDE.md's own summary of the 2026-08-21 audit ("no secret leakage... findings"). **Resolved 2026-08-26:** CLAUDE.md §19 (and its AGENTS.md counterpart) corrected to state this and the other three Part 0 blockers explicitly, rather than claiming none exist — the bugs themselves are unaffected and remain open; only the documentation's claim about them changed.
