...
2026-08-26: `BL-001`, `BL-002`, and `BL-003` fixed (code, not yet
committed/released — see their own entries); `BL-004` remains open.** Release readiness still
requires all four resolved, not just these two.
...
- **Target:** Done in code. Once committed, a further local commit on top of `v4.6.0`, not yet released — same as `BL-001`/`BL-002`'s siblings; `BL-004` remains open.
...
- **Decision:** Open. Immediate fix candidate. Not implemented this pass (investigation-only, per explicit instruction).
