...
def test_discover_candidates_recurses_into_a_differently_named_container(tmp_path):
    """
    Generalization: a container folder doesn't have to be named 'services'/
    'apps'/'packages' -- any directory that doesn't look like a service
    itself but has 2+ children that do (e.g. 'workspaces/') gets the same
    one-level-deeper treatment, naming-agnostic.
    """
    (tmp_path / "workspaces" / "api").mkdir(parents=True)
    (tmp_path / "workspaces" / "api" / ".env").write_text("KEY=1\n")
    (tmp_path / "workspaces" / "web").mkdir(parents=True)
    (tmp_path / "workspaces" / "web" / ".env").write_text("KEY=1\n")
...
def test_discover_candidates_does_not_recurse_into_a_container_with_only_one_service(
    tmp_path,
):
    """
    The generic (non-well-known-name) container heuristic requires 2+
    service-like children to distinguish a real container from an
    ordinary, unrelated directory that merely happens to hold one service
    -- a single nested service under a non-standard name is a known,
    accepted gap, not a false positive to guess at.
    """
    (tmp_path / "workspaces" / "api").mkdir(parents=True)
    (tmp_path / "workspaces" / "api" / ".env").write_text("KEY=1\n")
...
def test_find_config_source_skips_an_envshield_generated_dotenv_for_a_real_module(
    tmp_path,
):
    """
    A '.env' written by 'envshield setup' is a derivative of the schema --
    its values came from prompting against variables the schema already
    knew about -- not an independent source, so it should never outrank a
    genuine Python config module the way a hand-written '.env' would.
    """
...
def test_find_config_source_falls_back_to_a_generated_dotenv_when_nothing_else_exists(
    tmp_path,
):
    """A generated '.env' is still better than nothing to seed a schema from."""
    env_file = tmp_path / ".env"
    env_file.write_text(
        "# Auto-generated by 'envshield setup' on 2026-01-01\n\nSECRET_KEY=x\n"
    )

    assert service_discovery.find_config_source(str(tmp_path)) == str(env_file)


def test_looks_like_it_reads_the_environment_is_false_for_hardcoded_literals(
    tmp_path,
):
    ...
    assert service_discovery.looks_like_it_reads_the_environment(str(settings)) is False


def test_looks_like_it_reads_the_environment_is_true_for_os_getenv(tmp_path):
    settings = tmp_path / "settings.py"
...
def test_looks_like_it_reads_the_environment_is_true_for_pydantic_base_settings(
    tmp_path,
):
    settings = tmp_path / "settings.py"
    settings.write_text(
