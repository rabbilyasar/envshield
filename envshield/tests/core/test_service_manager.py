...
def test_resolve_service_auto_selects_the_only_configured_service(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    with open("envshield.yml", "w") as f:
        f.write("services:\n  athena:\n    path: athena/env.schema.toml\n")
...
def test_resolve_service_prompts_when_multiple_and_none_given(mocker, tmp_path, monkeypatch):
    """
...
def test_resolve_service_does_not_offer_all_when_disallowed(mocker, tmp_path, monkeypatch):
    _write_two_services(monkeypatch, tmp_path)
    mock_select = mocker.patch("questionary.select")
