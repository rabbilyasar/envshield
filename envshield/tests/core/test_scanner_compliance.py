...
def test_scan_with_undeclared_variable(mocker, tmp_path):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path) as td:
        ...
        # Assertions
        assert result.exit_code == 1, "Scan should fail if undeclared variables are found"
...
def test_scan_with_both_secret_and_undeclared_variable(mocker, tmp_path):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path) as td:
        ...
        with open(SCHEMA_FILE_NAME, "w") as f:
            f.write('[DECLARED_KEY]\ndescription="This one is okay"\n')
        mocker.patch("envshield.config.manager.load_schema", return_value={"DECLARED_KEY": {}})
...
def test_scan_gracefully_handles_missing_schema_file(mocker, tmp_path):
    """
    Edge Case: Tests that the scanner runs without crashing if the schema file
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path) as td:
        ...
        # Assertions
        assert result.exit_code == 0, "Scan should pass if no secrets are found, even without a schema"
        assert "Warning: Schema not found. Skipping undeclared variable check." in result.stdout
        assert "No issues found" in result.stdout
